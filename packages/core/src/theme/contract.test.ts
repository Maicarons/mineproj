...
describe('resolveThemeExtends', () => {
  it('returns the theme itself when it has no parent', () => {
    const resolved = resolveThemeExtends(parentTheme);
  ...
  it('inherits everything the child omits and keeps child overrides', () => {
    ...
    const resolved = resolveThemeExtends(child);
  ...
  it('follows string references through the loader callback', () => {
    const child: Theme = defineTheme({
      name: 'child-theme',
      extends: '@mineproj/theme-classic',
      layouts: { home: noop },
    });
    const resolved = resolveThemeExtends(child, (reference) => {
  ...
  it('resolves multi-level chains with nearest override winning', () => {
    ...
    const resolved = resolveThemeExtends({ name: 'leaf', extends: mid, layouts: {} });
  ...
  it('throws on circular extends chains', () => {
    ...
    expect(() => resolveThemeExtends(a)).toThrow(/Circular theme extends chain/);
  });

  it('throws when a string parent has no loader', () => {
    const child: Theme = defineTheme({ name: 'child', layouts: {}, extends: 'somewhere' });
    expect(() => resolveThemeExtends(child)).toThrow(/string references require resolveTheme/);
  });

  it('supports configSchema inheritance', () => {
    const child: Theme = defineTheme({
      name: 'child',
