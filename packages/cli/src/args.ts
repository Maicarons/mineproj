...
export interface ParsedArgs {
  command: 'dev' | 'build' | 'preview' | 'check' | 'info' | 'audit' | 'theme:eject' | 'new' | 'doctor' | 'migrate' | 'i18n:init' | 'i18n:extract' | 'editor:export' | 'help' | 'version';
  flags: CliFlags;
}
...
Commands:
  ...
  migrate              Schema version migration and backup
...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (!['dev', 'build', 'preview', 'check', 'audit', 'theme:eject', 'new', 'doctor', 'migrate', 'info', 'i18n:init', 'i18n:extract', 'editor:export', 'help', 'version'].includes(command)) {
